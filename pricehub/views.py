...
def _load_raw_data(price_model, expansion_code=None, rarities=None):
    """카드별 최신 raw_data — card.latest_raw_data 캐시 컬럼에서 직접 조회 (빠름)"""
    # price_model로부터 card 모델 추론
    card_model = price_model._meta.get_field('card').related_model

    qs = card_model.objects.filter(latest_raw_data__isnull=False)
    if expansion_code:
        qs = qs.filter(expansion__code=expansion_code)
    if rarities:
...
def _collect_mall_names(price_model, expansion_code=None, limit=500):
    """raw_data에서 mallName 빈도 집계 — card.latest_raw_data 캐시 컬럼 사용"""
    card_model = price_model._meta.get_field('card').related_model
