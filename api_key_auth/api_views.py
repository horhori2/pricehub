...
엔드포인트:
    ...
    GET /api/pokemon/kr/prices/target/latest/      최신 타겟 스토어 가격
...
def _card_queryset_with_latest_prices():
    return (
        ...
        .prefetch_related(
            ...
            Prefetch('target_prices', queryset=TargetStorePrice.objects.order_by('-collected_at')),
        )
    )
...
# ── 6. 최신 타겟 스토어 가격 ─────────────────────────────────
# GET /api/pokemon/kr/prices/target/latest/

class LatestTargetPriceListView(generics.ListAPIView):
    authentication_classes = [APIKeyAuthentication]
    permission_classes = [HasAPIKey]
    serializer_class = TargetStorePriceSerializer
    ...
    def get_queryset(self):
        ...
        hours = int(self.request.query_params.get('hours', 24))
        since = timezone.now() - timedelta(hours=hours)
        qs = TargetStorePrice.objects.filter(collected_at__gte=since).select_related('card', 'card__expansion')
        expansion_code = self.request.query_params.get('expansion')
        if expansion_code:
            qs = qs.filter(card__expansion__code=expansion_code)
        return qs
...
def price_collection_summary(request):
    ...
    target_rows = (
        TargetStorePrice.objects
        .filter(collected_at__gte=since)
        .values('store_name')
        .annotate(total=Count('id'), last_collected=Max('collected_at'))
