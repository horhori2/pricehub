...
class TargetStorePriceSerializer(serializers.ModelSerializer):
    class Meta:
        model = TargetStorePrice
        fields = ['id', 'price', 'store_name', 'collected_at']


class TargetStorePriceLatestSerializer(serializers.ModelSerializer):
    class Meta:
        model = TargetStorePrice
        fields = ['price', 'store_name', 'collected_at']
...
class CardListSerializer(serializers.ModelSerializer):
    ...
    latest_target_price = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            'latest_target_price',  # ← 최신 타겟 스토어가
        ]
    ...
    def get_latest_target_price(self, obj):
        target_prices = obj.target_prices.all()
        return TargetStorePriceLatestSerializer(target_prices[0]).data if target_prices else None


class CardDetailSerializer(serializers.ModelSerializer):
    """
    카드 상세용.
    naver_price_history / target_price_history 분리.
    """
    expansion = serializers.SerializerMethodField()
    naver_price_history = serializers.SerializerMethodField()
    target_price_history = serializers.SerializerMethodField()
    ...
    def get_target_price_history(self, obj):
        limit = self.context.get('price_limit', 30)
        return TargetStorePriceSerializer(obj.target_prices.all()[:limit], many=True).data
